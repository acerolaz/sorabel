...
| É10 | La spec §11.3 exige que l'acceptance soit jouée « à travers un vrai client MCP (session du SDK sur le transport HTTP streamable) contre l'application assemblée par `dependencies.py` — aucun accès aux objets internes, aucun monkeypatch ». Les scénarios livrés instancient `GovernedFastMCP` à la main, réenregistrent les tools avec les doublures, posent `request_ctx` eux-mêmes, appellent `serveur.call_tool()` en direct, et utilisent `monkeypatch` dans deux scénarios. Conséquence concrète, sans l'adoucir : appeler `serveur.call_tool()` court-circuite le handler bas niveau `tools/call`, donc les assertions d'audit de l'acceptance décrivent une séquence de journal qu'un vrai client ne produit pas — un vrai `tools/call` laisse en plus une ligne `list_tools:internal` (cf. `tests/integration/test_governed_http_session.py`) | La couverture protocolaire réelle (vrai uvicorn, vraie session SDK, vrai vérificateur de jetons, rejeu de session, et désormais le contrat d'erreur structuré sur le fil) est portée par `tests/integration/test_governed_http_session.py` ; la migration des scénarios d'acceptance vers le protocole reste à faire |
